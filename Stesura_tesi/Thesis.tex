...
\begin{algorithm}
	...
	\begin{algorithmic}[1]		
		...
		\State Choose $\sigma_{k}\in[\sigma_{min},\sigma_{max}]$; 
	\For{$k = 1,\dots$}
		\State Solve
		\State $\begin{bmatrix}
			0&A^{T}&I \\A&0&0\\X^{k}&0&S^{k}
...
~\cite{Wright} pag. 101
\end{proof}
In the general framework \textbf{Algorithm 1} we have not specified how the centering parameter $\sigma_{k}$ is choosen, as different interior-point algorithms use a peculiar method to select $\sigma_{k}$\\. The
method of compute  $\sigma_{k}$ distinguishes different path-following algorithms.\\
...
Now we see two heuristic choices of the centering parameter $\sigma_{k}$, with $k$ the current iteration of the algorithm; the first one decreases at each iteration and the second one is fixed: 
\begin{equation}
\begin{split}
\sigma_{k_{1}} &= \min\{0.1, 100\mu_{k} \},\\
\sigma_{k_{2}} &= 1 -\frac{0.5}{\sqrt{n}}
\end{split}
\end{equation}
with $\mu_{k}$ the duality measure of the current primal-dual point and $n$ is the number of entries of $x$ and $s$. 
Analyzing the graphic \ref{fig:float} we state that the \textbf{Algorithm 1} with the first choice of $\sigma_{k}$ requires a less number of iterations and less time storage to find a LP solution.\\
\begin{figure}
\begin{center}
...
\begin{array}{ | l | l | }
	\hline
	\text{LP problem} & \text{Constant K} \\ \hline
	0 &      2.006 \\ \hline
	1 &      5.537 \\ \hline
	2 &      0.736 \\ \hline
	3 &      3.152 \\ \hline
...
\begin{thebibliography}{9}
	...
	\bibitem{Lem} Lemke C.E, \textit{ The dual method of solving the linear programming problem.} Naval Research Logistics Quarterly, John Wiley and Sons, (1954).
		...
		\bibitem{meg} Megiddo N., \emph{ "Pathways to the optimal set in linear programming, in Progress in
			Mathematical Programming"}, Springer-Verlag, New York, pp. 131-158, (1986).
	\bibitem{MER} Mehrotra S., \emph{ On the implementation of a primal-dual interior point method. } SIAM J on Optimization, 2, pp: 575-601 (1992).
