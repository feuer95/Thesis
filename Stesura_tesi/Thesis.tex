...
We consider the LP problem formulated in the standard form $\ref{(Prim)}$. \\
...
\end{center}
where $\mathit{J}$ is the Jacobian of the linear mapping $\mathit{F}$. For $(x, \lambda, s)\in\mathcal{F}^{0}$, the direction $(\Delta x, \Delta \lambda, \Delta s)$ satisfies
\begin{equation}
\begin{bmatrix}
0&A^{T}&I \\A&0&0\\S&0&X
...
\end{bmatrix}.
\end{equation}
 If the current point $(x, \lambda, s)\notin\mathcal{F}$, the Newton step equations become
...
0&A^{T}&I \\A&0&0\\S&0&X
	...
	\end{bmatrix},
\end{equation}
where the residuals $r_{b}$ and $r_{c}$ are defined respectively by $r_{b} = Ax - b$ and $r_{c} = A^{T}\lambda + s-c$.\\
...
\begin{equation}\label{log}\tag{P$_{\tau}$}
\begin{split}
\text{minimize\;} &c^{T}x + \tau\sum_{i=0}^{n}\ln{x_{i}}\\
\text{subject\;to\;}&Ax = b\;\text{and\;} x\geq0.
\end{split}
\end{equation}
In fact, the Lagrangian function for (\ref{log}), with Lagrange multipliers $\lambda$ and $s$ for the equality constraint, is
\begin{equation}
...
s&\geq 0.\\
\end{align*}  
Since the objective function is strictly convex, these conditions are sufficient as well as necessary for optimality; besides, defining $s_{i} = \dfrac{\tau}{x_{i}}$, we recover (\ref{(Tao)}): it imposes that the pairwise products $x_{i}s_{i}$ have the same value for all indices $i$.\\[0.5cm]
...
\begin{prop}
	Assume that the LP problem (P) is feasible. Then the set of optimal solutions of (P) is non-empty and bounded if and only if $\mathcal{D}^{0} = \mathcal{D}\cap\{x\in\mathbb{R}^{n}| s> 0\}$  is non-empty.
\end{prop}
The proof is an application of duality theory for LP. As a consequence of the two previous propositions, we have the following corollary.
\begin{cor}
...
In this research we discuss the \textit{Long-step Path-Following algorithms} (LPF), that selects the second neighborhood. Since  $\mathcal{N}_{-\infty}(\gamma)$ is a larger central path neighborhood, it allows for more flexibility in the choice of the next iterate and hence larger step length (hence, the name long-step method). After computed the search direction with the Newton method, the step length $\alpha_{k}$ is chosed as the maximum value subject to staying inside $\mathcal{N}_{-\infty}(\gamma)$.\\
The following Lemma shows important properties of the points in $\mathcal{N}_{-\infty}(\gamma)$.
It is stated that the sequence $\{x^{k},s^{k}\}$ generated by a LPF algorithm is bounded, hence there is at least one limit point $(x^{*},s^{*})$ for $\mu_{k}\to0$. Defining $\lambda^{*}$ the vector such that $c-s^{*}= A\lambda^{*}$, the limit points $(x^{*},\lambda^{*},s^{*})$ correspond to the strictly complementary solutions for which:
\begin{align*}
&x^{*}_{i}>0, \text{\;for\;} i \in\mathcal{B},
&s^{*}_{i}>0, \text{\;for\;} i \in\mathcal{N},
\end{align*}
where $\mathcal{B}\cup\mathcal{N}$ is the partition of $\{1,2,\dots,m\}$ defined in (\ref{index}).
\begin{lem}
	Let $\mu_{0}\geq 0$ and $\gamma\in(0,1)$. Then for all points $(x,\lambda,s)$ such that
	\begin{equation*}
...
%	\textbf{end}
%\end{tabbing}
The centering parameter $\sigma_{k}$ is bounded by $\sigma_{\text{min}}$ and $\sigma_{\text{max}}$; in this research we see two practical LPF algorithms with a specific $\sigma$. \\

\subsection{{Asymptotic convergence}
We examine the asymptotic behavior of these primal-dual methods, that is of significant theoretical interest. \\
We see that $\Delta x$ and $\Delta s$ compnents of the affine-scaling step have size $\mathcal(O)(\mu)$. Instead the algorithm LPF generates a sequence such that converges almost quadratically to zero.\\
The predictor-corrector methods, presented in the suceeding chapter, enhances the algorithms before described.   
The theoretical properties 
the we assume that $(x, \lambda, s)\in\mathcal{F}^{0}$, hence, the affine-scaling direction $(\Delta x, \Delta \lambda, \Delta s)$ satisfies the following matricial system:\\
\begin{equation}\label{5.1}
\begin{bmatrix}
0&A^{T}&I \\A&0&0\\S&0&X
\end{bmatrix}\begin{bmatrix}
...
\subsection*{LPF methods convergence}
...
For all points in the neighborhood $\mathcal{N}(\gamma,\beta)$, the infeasibility is uniformly bounded by some multiple of the duality measure $\mu$. By forcing $\mu_{k}$ to zero monotonically and restricting all iterates $(x_{k},\lambda_{k},s_{k})$ to the neighborhood $\mathcal{N}(\gamma,\beta)$, we have that the residuals $r_{b}^{k}$ and  $r_{c}^{k}$ tend to 0. The other condition in $\ref{neigh3}$ ($x_{i}s_{i} \geq \gamma\mu$) is the same as in the feasible neighborhhod $\mathcal{N}_{-\infty}(\gamma)$. It keeps the pairwise products $x_{i}s_{i}$ roughly in balance and prevents the Newton-like search directions from being distorted by components of $(x,s)$ that approach zero prematurely.\\
A result similar to \textbf{Theorem} $\ref{(Z)}$ holdds for the steps of an infeasible interior-point algorithm. We state the result in general terms in the following theorem:
\begin{thm}
	Suppose that the iteration sequence $(x^{k}, \lambda^{k},s^{k})$ is generated by an infeasible-interior point algorithm whose iterates are confined to the neighborhood $\mathcal{N}(\gamma,\beta)$ defined by $\ref{neigh3}$, 
\end{thm}
...
\begin{align}\label{Qw}\tag{4.23a}
\alpha_{\text{aff}}^{\text{pri}}=\arg\max\{\alpha\in[0,1]\;|\;x +\alpha\Delta x^{\text{aff}}\geq 0\}, \\
\alpha_{\text{aff}}^{\text{dual}}=\arg\max\{\alpha\in[0,1]\;|\;s +\alpha\Delta s^{\text{aff}}\geq 0\}\tag{4.23b}.
\end{align}
...
\begin{equation}\tag{4.23c}\label{CP}
...
\begin{tabbing}
	...
	\> calculate $\alpha_{\text{aff}}^{\text{pri}}$, $\alpha_{\text{aff}}^{\text{dual}}$ and $\mu_{\text{aff}}$ as in (4.23);\\
...
\begin{align}\label{QWw}\tag{4.31a}
\alpha_{\text{aff}}=\arg\max\{\alpha\in[0,1]\;|\;(x^{k}, \lambda^{k}, s^{k})+ \alpha(\Delta x^{\text{aff}}, \Delta\lambda^{\text{aff}}, \Delta s^{\text{aff}})\in\mathcal{N}_{-\infty}(\gamma,\beta)\}.
\end{align}
...
\begin{equation}\tag{4.31b}
\mu_{\text{aff}}= (x+\alpha_{\text{aff}}\Delta x^{\text{aff}})^{T}(s+\alpha_{\text{aff}}\Delta s^{\text{aff}})/n.
\end{equation}
\begin{algorithm}\caption{\label{alg:pc}PC-LPF algorithm}
\begin{tabbing}
	\\
	\textbf{Given} $(x^{0}, s^{0})> 0$; \\
	\textbf{for} \= $k = 0, 1, 2,...$ \\
	\> solve (\ref{(A)}) for $(\Delta x^{\text{aff}},\Delta \lambda^{\text{aff}},\Delta s^{\text{aff}})$;\\
	\> calculate $\alpha_{\text{aff}}$ and $\mu_{\text{aff}}$ as in (4.31);\\
...
In the convergence analysis of the IPM, it is assumed the starting point $(x^{0}, \lambda^{0}, s^{0})$ is a primal-dual feasible point in ${F}$, but it is required only $(x^{0},s^{0})>0$.\\
The theorem $TO WRITE$ ensures convergence under loose restrictions on the starting point .
