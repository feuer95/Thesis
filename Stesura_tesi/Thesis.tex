...
La ricerca pone particolare attenzione sul ruolo del "predictor-corrector step" nel metodo IPM di Mehrotra e attua questa strategia al metodo \textit{Long-Path Following} (LPF), il quale si differenzia dagli altri  IPM operando su un intorno limitato $\mathcal{N}_{-\infty}(\gamma,\beta)$.\\
...
We examine the asymptotic behavior of these two methods, that are of significant theoretical interest: the following complexity results state that IPM converge arbitrarily close to an optimal solution linearly respect to the duality measure $\mu$. We will develop a modification of the LPF algorithm, in order to be accelereted so that its sequence of duality measure $\{\mu_{k}\}$ converges almost quadratically to 0. This modified method is depicted in Section 4.5.2.\\
...
\begin{proof}
	...
	\end{equation*}	
\end{proof}
This marginal per-itertion reduction of $\mu^{k}$ is enough to deliver a polynomial complexity of the logarithm \cite{25y}.\\
...
%\textcolor{red}{DA COMPLETARE}\\
%\end{center}
In this chapter it is developed a comprehensive analysis of the results obtained by some practical implementations. It is presented three LP problems: the forest service allocation, the Swedish steel model, tubular products operations planning.\\
The fictitious data used to test the algorithms come from \cite{RR}.
The alfgorithms are recalled with the tags used in the previous chapter.
...
\begin{figure}\caption{\label{figure:for}Execution of IPM: }
...
\begin{table}[]\caption{\label{table:PNV}Numerical results obtained by my codes.}
	\begin{tabular}{cclcll}
		\hline		\textbf{} & \textbf{Initial point} & \multicolumn{1}{c}{\textbf{Optimal value}} & \textbf{Iteration number} & \multicolumn{1}{c}{\textbf{$\mu$}} & \multicolumn{1}{c}{\textbf{Residual} Res} \\ \hline
		...
		\multicolumn{1}{r|}{\multirow{2}{*}{\textbf{PC-LPF}}} & MIP & 3.225150E+05 & 18 & 1.857490E-08 & 7.722891E-10 \\
		\multicolumn{1}{r|}{} & STP1 & 3.225150E+05 & 16 & 1.946073E-08 & 4.272252E-09 \\ \hline
...
For each method it is plotted two graphics. In the first one we illustrate the behaviour of the duality measure, dual gap and the centering deviation, defined respectively in (\ref{dualgap}) and in (\ref{cendev}). 
The affine algorithm with both two starting points strategies requires all iterations storage to achieve the feasible region (see (b) and (d)). Instead, the sequence computed by LPF 1 reaches the feasible region after 25 iterations, although the computational cost is higher: the rate of speed of duality gap doesn't improve.  
In the LPF 2 the improvement of g is noticeable: when the priaml-dual point sequence approaches to the central path $\mathcal{C}$ and to the feasible region, the duality measure tends to 0 with more velocity.\\ 
...
We check that when the centering deviation grows (and the sequence moves away from $\mathcal{C}$), the duality gap's velocity increases. 
\newpage
...
\chapter{Conclusion\textcolor{red}{ DA COMPLETARE}}
