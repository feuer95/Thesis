...
\end{equation}
 ...
 \\Note that for any $x$ we have $x_{B}=A_{B}^{-1}b-A_{B}^{-1}A_{N}x_{N}$ and the cost function $c^{T}x=c_{B}A_{B}^{-1}b+(c_{N}-c_{B}A_{B}^{-1}A_{N})\;x_{N}$, denoting the \textit{reduced cost} by $\widetilde{c}=c_{N}-c_{B}A_{B}^{-1}A_{N}$.\\ Since we are dealing only with basic feasible points, we consider the relative basic feasible point $x$ with $x_{N}= 0$ and the cost value it is equal to $\widetilde{c}$.\\
