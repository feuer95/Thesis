...
an introductory section summarizes the basic theory of the linear programming. In the chapter 2 we delineate the simplex method followed by a sensitivity analysis. The chapter 3 is dedicated to the primal-dual interior point methods, taking particular attention to the affine-scaling and the long-path following methods. In this section it is also defined the Mehrotra's predictor-corrector algorithm, which is the basis of much of the current generation of software, even if no convergence theory is available for this algorithm.\\The chapter 7 is devoted to the implementation of the algorithms discussed in the
previously: we present the numerical results that we obtained for the considered physical problems and a comparison is performed.
Concluding, in chapter 7 we give an overview of the work realized in this thesis and
...
\end{equation*}
To calculate the centering step component, we solve a linear system with the same coefficient matrix as in \ref{(A)} and right-hand side $(0, 0,\sigma\mu e)$.\\
...
When the limiting matrix is singular, however, the corrector step may no longer be smaller in norm than the affine-scaling step, actually, it is often larger. Even in this situation, the use of the corrector component usually enhances the overall efficiency of the algorithm in practice.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]		\State Given: $(x^{0}, \lambda^{0}, s^{0}) > 0$
		\For{$k = 1,\dots$}
...
\begin{center}\label{T}
	$\begin{bmatrix}
	A^{T}\lambda+s-c \\Ax-b \\XSe
	\end{bmatrix}=\begin{bmatrix}
	(1-\tau)(A^{T}\lambda-c)\\(1-\tau)(Ax-b)\\(1-\tau)XSe
	\end{bmatrix}$, with $(x,s)\geq0$.
\end{center}
...
\end{equation*}
Here, $(x_{0}^{j},\lambda_{0}^{j},s_{0}^{j})$ is the derivative of $(x_{\tau},\lambda_{\tau},s_{\tau})$ with respect to $\tau$, evaluated in $\tau = 0$. We can find these derivatives by implicity differenziating the equation \ref{T}:
\begin{equation}
\begin{bmatrix}
0&A^{T}&I \\A&0&0\\X&0&S
\end{bmatrix}\begin{bmatrix}
