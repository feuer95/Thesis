...
This thesis introduces the two major types of methods: the Simplex method and the Interior-Point Methods (IPMs).
...
results demonstrate the benefits of corrector approach in LPF method as well as the reliability of the
method.
...
In this research is proposed a new long-path following algorithm labeled as \textit{predictor-corrector long-path following algorithm}, based on a new centering parameter computation. 
. The initial point choice and the
termination criteria will be explained in the next chapter. With practical implementations a comparison will be discussed.
...
We test the algorithms on 30 small scale LP problems in standard form with model data $A\in\mathbb{R}^{m\times n}$, $b\in\mathbb{R}^{m}$ and $c\in\mathbb{R}^{n}$. In the Table \ref{tab:numite} we collect statistics from the runs, summarizing the number of iterations $T_{k}$ with $k =1,\dots,30$.
...
\begin{figure}\label{figure:T}
	\begin{center}
		\includegraphics[width= 17 cm]{numberiterations}\caption{log–log plot of T vs. $m + n$ and the L1 regression lines.}
	\end{center}
\end{figure}
%\ref{figure:T}
...
Now we outline the geometric viewpoint of the simplex method and the IPMs, assuming the LP problem is in canonical form.\\
