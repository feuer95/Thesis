...
where the $b_{j}$, $c_{i}$ \text{and} $a_{ij}$ are fixed real costants, with $i \in\{1,...,m\}$ and $j\in\{1,...,m\}$.\\ In a more compact notation, using vectors, the standard problem can be written as
...
\section*{Initialization and termination}
In the discussion of the interior point methods, it is assumed the starting point $(x_{0}. \lambda_{0}, s_{0})$ is feasible respect to the linear equations (2.4) and (2.5).\\
...
The choice of starting point is an important practical issue with a significant effect on the robustness of the algorithm. A popular heuristic for finding $(x_{0}. \lambda_{0}, s_{0})$ starts by calculating $(\tilde{x}, \tilde{\lambda}, \tilde{s})$ as solution of two least-squares problems:
\begin{align*}
...
\end{align*}
There is an algorithm, termed \textit{Homogeneous Self-Dual Algorithm} (HSD) that overcomes to the difficulties mentioned above.\cite{XY}\\
A LP is said \textit{self-dual} if the dual of the problem is equivalent to the primal. The advantage of self-duality is that we can apply a primal-dual interior-point algorithm to solve the self-dual problem without doubling the dimension of the linear system solved at each iteration.\\
The algorithm HSD is based on the construction of a homogeneous and self-dual linear programming problem in such a way the point $x=1, \lambda = 0, \tau= 1,z=1,\theta= 1$ is feasible. The primal LP is
\begin{alignat*}{3}
\min&            &      & & (n+1)\theta&\\
\text{subject to }&&Ax&-b\tau&+\bar{b}\theta&=0,\\
-&A^{T}\lambda& &+c\tau&-\bar{c}\theta&=0,\\
&b^{T}\lambda&-c^{T}x& &+\bar{z}\theta&\geq0,\\
-&\bar{b}\lambda&+\bar{c}^{T}x&-\bar{z}\tau&&=-(n+1),\\
\lambda \text{ free, }x\geq0,\tau\geq0, \;\theta \text{ free,} 
\end{alignat*}  
where
\begin{equation*}
\bar{b} = b -Ae,\;\; \bar{c}=c-e,\;\;\bar{z}=c^{T}e+e.
\end{equation*}
The vectors $\bar{b}$,$\bar{c}$ and $\bar{z}$ are chosen so that the system associated to the LP is feasible: for example $(x,\lambda, \tau,\theta)=(e^{T},0,e^{T},e^{T})$ with $e$ unit vector. We now relate optimal solutions to HSD to those for LP. Let us state, without proof, that the HSD problem has an optimal solution and its optimal solution set is bounded. \cite{LNP}
...
\bibitem{KY}Xiaojie Xu, Yinyu Ye, \emph{"A generalized homogeneous and self-dual algorithm for linear programming"}, Operations Research Letters, 17, pp: 181-190, (1995)
