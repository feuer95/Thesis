...
\begin{algorithm}[H]
	...
	\end{tabbing}
	\begin{equation}\label{Pb}\tag{4.5}
	\begin{bmatrix}
	0&A^{T}&I \\A&0&0\\X^{k}&0&S^{k}
	\end{bmatrix}\begin{bmatrix}
	\Delta x^{k}\\\Delta\lambda^{k} \\\Delta s^{k}
...
\begin{equation}\label{neigh3}\tag{4.6}
...
\begin{equation}\label{matrix:LPF}\tag{4.7}
\begin{bmatrix}
0&A^{T}&I \\A&0&0\\X^{k}&0&S^{k}
\end{bmatrix}\begin{bmatrix}
\Delta x^{k}\\\Delta\lambda^{k} \\\Delta s^{k}
...
\begin{lem}
	...
	\begin{enumerate}
		...
		&A_{\mathcal{N}^{T}}\pi+ v =0.\label{45}\tag{4.8c}		 
		\end{align}
