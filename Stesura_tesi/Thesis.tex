...
Another strategy aimed to satisfy the requirement that the starting point is both well centerd and not to infeasible is presented in \cite{MER}.\\
This popular heuristic approach for finding $(x^{0}, \lambda^{0}, s^{0})$ consists on computing least squares solutions to the equations corresponding to the linear constraints of the LP and then shifting the values of the variables to ensure that they are kept away from the boundary of the orthant. Now we see in detail.\\
...
\end{align*}
\\
The logical basis for this choice is that many path-following methods are able to perform long steps toward the solution and to drive the infeasibility to zero at least at the same rate as the duality gap, by keeping the iterates in the central path neighbourhood \begin{equation*}
\mathcal{N}(\gamma,\beta) =\Bigg\{(x, \lambda, s) | \frac{\lVert(r_{b}, r_{c})\rVert_{2}}{\mu} \leq \beta\frac{\lVert(r_{b}^{o}, r_{c}^{o})\rVert_{2}}{\mu^{o}}, (x, s)>0, x_{i}s_{i} \geq \gamma\mu,\;i = 1,\dots, n \Bigg\}
\end{equation*}
where $\gamma\in(0,1)$ and $\beta \geq 1$ are given parameters and $(r_{b}^{o}, r_{c}^{o})$ and $\mu_{o}$ are evaluated at the starting point $(x^{o}, \lambda^{o}, s^{o})$. Note that we must have $\beta \geq 1$ to ensure that the initial point $(x^{o}, \lambda^{o}, s^{o})\in\mathcal{N}(\gamma,\beta)$. \cite{SPS}
\\
\\
...
(x^{o}, \lambda^{o}, s^{o}) = \eta \tilde{w}
\end{equation*}where $\tilde{w} = (e,0,e)$ and $\eta$ such that 
\begin{equation}\label{su}
\frac{\lVert(r^{o}_{b},r^{o}_{c})\rVert_{2}}{\mu^{o}}\leq \tau<1. 
\end{equation}
It easy to verify that, since $\mu^{o}$ increases quadratically with $\eta$ and $\lVert(r^{o}_{b},r^{o}_{c})\rVert_{2}$ incrreases linearly, inequality \ref{su} approximately holds if $\eta$ satisfies
\begin{equation*}
...
We note that $(x^{o},\lambda^{o},s^{o})$ is perfectly centeredand all its componenta with lower bound 0 are "sufficient" positive, but even if the ratio in \ref{suu} is small, the duality gap and the infeasibility may very large, slowing the progress toward the solution.\\
This method strategy, labeled with STP1 (\cite{SPS}), is summarized in \ref{fig:STP1}
\begin{figure}
	\caption{\label{fig:STP1}}
\end{figure}
\newpage
