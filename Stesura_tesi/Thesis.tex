...
\begin{align*}
|c^{T}x-b^{T}\lambda|=|y^{T}Ax+s^{T}x-r_{c}^{T}x+r_{b}^{T}\lambda-x^{T}A^{T}\lambda|\geq |s^{T}x|.
\end{align*}
...
In the theoretical analysis of IPM it is assumed the starting point $(x^{0}, \lambda^{0}, s^{0})$ satisfies the linear equations (\ref{DF}) and (\ref{PF}).\\In practice there is no need to impose the feasibility of all iterates and force the algorithm to stay in the primal-dual feasible set $\mathcal{F}$.\\
...
\end{equation*}
to compute the positive vectors $\hat{x}= \tilde{x}+\delta_{x}e$ and $\hat{s}= \tilde{s}+\delta_{s}e$, where $e$ is the unit vector. To ensure that the initial points $x^{0}$ and $s^{0}$ are not too close to zero, we add two more scalars defined as follows:
...
\end{equation*}where $\tilde{w} = (e,0,e)$ and $\eta$ is the value such that the residuals$r_{b}^{0}$ and $r_{c}^{0}$ and the dualitiy measure $\mu_{0}$ of this starting point satisfy the following:
...
\subfloat[PC LPF algorithm ]{\includegraphics[width= 7 cm]{for_PCLPF}} \qquad%
\subfloat[PC LPF algorithm]{\includegraphics[width= 7 cm]{for_PCLPFb}}\\
\subfloat[PC LPF algorithm ]{\includegraphics[width= 7 cm]{for_PCLPF2}} \qquad%
\subfloat[PC LPF algorithm]{\includegraphics[width= 7 cm]{for_PCLPF2b}}\\
...
\begin{table}[h]
	...
	\end{tabular}\caption{\label{tab:TP}Tubular products model data.}
\end{table}
\end{center}
...
Third point: The use of the predictor-corrector technique is advantageous in long-path foloowing metho: it leads to significant saving in the number of iterations. we show that the proposed correcting scheme in LPF method can occasionally fail: you can opt for weel-suited the neighborood parameters to remedy such difficulties.
%\begin{appendices}
\appendix
\chapter*{Appendix}	
