...
\begin{itemize}
	...
	\item the duality measure $\mu = \frac{x^{T}s}{n}$ decreases at each iteration;
...
\end{align}
in which $\mu_{\text{aff}}$ measure the efficiency of the affine-scaling direction.
...
\end{tabbing}
\end{algorithm}
Instead the corrector step consits on solving \\
 then the values of the step lengths and the centering parameter are performed in order to guarantee that the next iterate $(x^{k+1}, \lambda^{k+1}, s^{k+1})\in\mathcal{N}_{-\infty}(\gamma,\beta)$ are:
\newpage
...
\section*{Termination code}
 \lstinputlisting[language=Python]{term.py}
	...
	Let be $f \colon \mathbb{R}^{n} \to \mathbb{R}^{n}$ a continuously differentiable function and let $J(x)$ be
	the Jacobi matrix attached to $f$. Consider the system:
\begin{equation*}
f(x)=0.
\end{equation*}
Suppose we are given the vector $x^{0}$, then we obtain a sequence of points
using the formula:
\begin{equation*}
x^{k+1} = x^{k} - J(x^{k})^{-1}f(x^{k}).
\end{equation*}
If we introduce a step direction vector $\Delta x^{k}$, thus
\begin{equation*}
x^{k+1} = x^{k} + \Delta x^{k},
\end{equation*}
and we have
...
\end{equation*}
If $x^{0}$ is sufficiently close to a solution of f, then this sequence is convergent.
The Newton’s method is very important to solve the linear system associated with the IPMs.
%\end{appendices}
